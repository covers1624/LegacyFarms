...
public class RenderMill implements IBlockRenderer {
	...
	public void renderTileEntityAt(TileEntity tileentity, double d, double d1, double d2, float f) {
		TileMill mill = (TileMill) tileentity;
		render(mill.progress, mill.charge, ForgeDirection.WEST, gfxBase, d, d1, d2);
	}
	...
	private void render(float progress, int charge, ForgeDirection orientation, String gfxBase, double x, double y, double z) {
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(gfxBase + "pedestal.png"));
		pedestal.render(factor);

		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(gfxBase + "column_" + charge + ".png"));
		column.render(factor);

		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(gfxBase + "extension.png"));
		extension.render(factor);

		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(gfxBase + "blade1.png"));
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(gfxBase + "blade2.png"));
		GL11.glTranslatef(-translate[0] * tfactor, translate[1] * tfactor, -translate[2] * tfactor);
